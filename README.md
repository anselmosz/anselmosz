## Olá! Eu sou Matheus Anselmo 👋

🎓 Estudante de Engenharia de Software
💻 Desenvolvimento de Software • APIs • Automação • Cloud
🚀 Em busca de oportunidades em Tecnologia

### Sobre mim
Estudante de Engenharia de Software e Técnico em Desenvolvimento de Sistemas, com experiência profissional em suporte técnico para plataforma SaaS.

Tenho interesse em desenvolvimento de software, APIs, integrações entre sistemas, automação de processos e computação em nuvem. Atualmente desenvolvo projetos utilizando JavaScript, Node.js, SQL e Docker, buscando aplicar boas práticas de arquitetura, documentação, versionamento e resolução de problemas.

Gosto de transformar necessidades de negócio em soluções técnicas, sempre com foco em aprendizado contínuo, colaboração e melhoria de processos. 

Busco constantemente inovação, aprendendo novas tecnologias e aplicando-as em projetos práticos. Estou aberto a conexões, colaborações e oportunidades na área de desenvolvimento backend e cloud computing.

### Contato
- ***LinkedIn:** [anselmomatheus](https://www.linkedin.com/in/anselmomatheus/)
- **Email:** [anselmoma2005@outlook.com](mailto:anselmoma2005@outlook.com)

### Experiência

**Analista de Suporte Técnico Jr | Octadesk (LWSA)**

- Atendimento técnico e resolução de incidentes em plataforma SaaS
- Análise de integrações via API e webhooks
- Configuração de recursos do Meta Business
- Testes de funcionalidades e validação de cenários
- Documentação técnica e compartilhamento de conhecimento
- Colaboração com times de Produto e Engenharia

---

### Tecnologias e Ferramentas

#### 🎨 Frontend
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)

#### Backend
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![REST APIs](https://img.shields.io/badge/REST-API-blue?style=for-the-badge)

#### ☁️ Cloud e DevOps
![Docker](https://img.shields.io/badge/Docker-0073F7?style=for-the-badge&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-0F0F0F?style=for-the-badge&logo=linux&logoColor=white)

#### 🗄️ Banco de dados
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-003545?style=for-the-badge&logo=postgresql&logoColor=white)
![MariaDB](https://img.shields.io/badge/MariaDB-001545?style=for-the-badge&logo=mariadb&logoColor=white)

#### ⚙️ Ferramentas
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-F73100?style=for-the-badge&logo=postman&logoColor=white)

---

### 📈 Estatístcas

<p>
  <img 
    align="left" 
    width="48%"
    src="https://github-readme-stats-sigma-five.vercel.app/api?username=anselmosz&show_icons=true&theme=dark&include_all_commits=true&locale=pt-br&cache_seconds=86400"
  />

  <img 
    align="left" 
    height="200"
    src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=anselmosz&theme=dark&layout=compact&custom_title=Tecnologias&langs_count=9&cache_seconds=86400"
  />
</p>

